...
from Tharaabot.__main__ import (IMPORTED, HELPABLE, MIGRATEABLE, STATS, USER_INFO, DATA_IMPORT, DATA_EXPORT, CHAT_SETTINGS,
                             USER_SETTINGS)
from Tharaabot.modules.helper_funcs.chat_status import sudo_plus, dev_plus
...
def load(bot: Bot, update: Update):
    ...
    try:
        imported_module = importlib.import_module("Tharaabot.modules." + text)
    except:
...
def listmodules(bot: Bot, update: Update):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("Tharaabot.modules.", 1)[1]
